...
function load_cookie() {
  // Try localStorage first (works in Electron), fall back to cookies
  var settingsStr = localStorage.getItem("settings");

  if (!settingsStr) {
    // Fall back to cookie for browser compatibility
    ...
    if (allCookies.indexOf("settings") > -1) {
      ...
      settingsStr = settingsCookie.split("=")[1];
    }
  }

  if (settingsStr) {
    document.getElementById("remembersettings").checked = true;
    let settings = settingsStr.split("|").map((x) => x.split("-"));

    settings.forEach((setting) => {
      switch (setting[0]) {
        case "m":
          switch (setting[1]) {
            case "M": document.getElementById("mapyes").checked = true; break;
            case "C": document.getElementById("mapsmall").checked = true; break;
            case "V": document.getElementById("mapvertical").checked = true; break;
            case "N": document.getElementById("mapno").checked = true; break;
          }
          break;
        case "cc":
          switch (setting[1]) {
            case "N": document.getElementById("chestcolorno").checked = true; break;
            case "Y": document.getElementById("chestcoloryes").checked = true; break;
          }
          break;
        case "s":
          if (setting[1] === "Y") document.getElementById("sphereyes").checked = true;
          break;
        case "a":
          const autotrackingSetting = setting[1][0];
          const autotrackingPort = setting[1].substring(1);
          switch (autotrackingSetting) {
            case "Y": document.getElementById("autotrackingyes").checked = true; break;
            case "D": document.getElementById("autotrackingnodungeon").checked = true; break;
            case "N": document.getElementById("autotrackingno").checked = true; break;
          }
          if (autotrackingPort) document.getElementById("autotrackingport").value = autotrackingPort;
          break;
        case "ah":
          if (setting[1]) document.getElementById("autotrackinghost").value = setting[1];
          break;
        case "p":
          document.getElementById("spriteselect").value = setting[1];
          break;
        case "ms":
          if (setting[1] === "O") document.getElementById("oldmapstyles").checked = true;
          break;
        case "sc":
          switch (setting[1]) {
            case "D": document.getElementById("scale200") && (document.getElementById("scale200").checked = true); break;
            case "E": document.getElementById("scale150").checked = true; break;
            case "F": document.getElementById("scale100").checked = true; break;
            case "T": document.getElementById("scale75").checked = true; break;
            case "H": document.getElementById("scale50").checked = true; break;
            case "Q": document.getElementById("scale25").checked = true; break;
          }
          break;
        case "ad":
  ...
  // Restore marker size slider
  var savedMarkerSize = localStorage.getItem("markerSize");
  if (savedMarkerSize) {
    var slider = document.getElementById("markersizerange");
    if (slider) {
      slider.value = savedMarkerSize;
      document.getElementById("markersizevalue").innerText = savedMarkerSize + "px";
    }
  }
}
...
function launch_tracker() {
  ...
  // Save marker size preference
  var markerSize = document.getElementById("markersizerange").value;
