(function (window) {
  ...
  let outstandingUpdate = false,
    awaitingNextUpdate = true,
    awaitingResponse = false,
    enableAutoSave = false,
    tempSaveFile = null,
    tempCustomizerFile = null,
    initWidth = 0,
    initHeight = 0,
    currentDungeon = -1,
    currentPath = null,
  ...
  let ownItems = {},
    reachableEdges = null,
    showOverworldModeSection = false,
    showFullMapMain = true,
    expandedMainTable = true,
    showChecklist = false,
    currentOWPath = null,
    currentOWScreen = null,
    fullOverworldMode = null,
    fullOWFixedEdge = null,
    fullOWSelectedScreen = null,
    fullOWSelectedEdge = null,
    fullOWConnectorStart = null,
    fullOWConnectorEnd = null,
    extraOWScreen = null,
    extraOWDirection = null,
    fullOWPath = null,
    hoveredPathNumber = -1,
  ...
  let searchResults = new Map(),
    ...
    searchResultPaths = [],
    clickedOverworldPathIndex,
    clickedOverworldPathListName,
    clickedOverworldPath = null,
    clickedConnectorIndex,
    drawDarkWorldMain = false,
    drawDarkWorldPopout = false,
    mapModeMain = "single",
    mapModePopout = "single",
    mapModeAutoMain = true,
    mapModeAutoPopout = true,
    fullZoomMain = 0.8,
    fullZoomPopout = 0.8,
    fullZoomAutoMain = true,
    fullZoomAutoPopout = true,
    pathZoom = 1,
  ...
  const checkboxFlagsDisplay = [
    "settingsinvertdirectioncolors",
    "settingsrounddirections",
    "settingsrestorewindowsize",
    "settingsautosavesafety",
    "settingshidetopbar",
    "settingspreventclickedgescreen",
    "settingsdisableblur",
    "settingssearchmirrorportaldefault",
    "settingsomitforeignsymbols",
    "showmorerooms",
    "splitpath",
    "owsidebaredit",
    "owsidebarnew",
  ...
  window.loadOverview = function () {
    ...
    if (
      ...
      document.getElementById("owmain").style.display != "none" ||
      document.getElementById("owpathedit").style.display != "none" ||
      document.getElementById("owsearch").style.display != "none"
    )
      switchScene("overview");
    if ((!welcomeMode && document.getElementById("itemsync").checked) || document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) testConnection(true);
  };

  window.loadDungeonSummaries = function () {
    for (let n = 0; n < 13; n++) loadDungeonSummary(n);
  };
  ...
  window.loadDungeonSummary = function (n) {
    if (dungeonPaths[n].completed) {
      ...
      if (visibleSidebar) document.getElementById("sidesummary" + n).innerHTML = "<div class='symbolnode' style='background-image: url(" + "./images/dungeons/keychest0.png);'></div>";
    } else {
      ...
      for (let path of dungeonPaths[n].paths) {
        for (let l = 0; l < path.length - 1; l += 2) {
          if (path[l] === "i" || path[l] === "b") {
            ...
            if ((excludesmallkeys && id === "i0") || (excludebigkeys && id === "i1")) continue;
            if (!items.includes(id)) items.push(id);
          }
        }
      }
      if (items.length <= 2) {
        for (let l = 0; l < items.length; l++) s += createSymbolNode(symbolMap[items[l]], l * 32, 0);
      } else if (items.length <= 5) {
        s += createSymbolNode(symbolMap[items[0]], 0, 0);
        for (let l = 1; l < 3; l++) s += createSymbolNode(symbolMap[items[l]], (l + 1) * 16, 0, true);
        for (let l = 3; l < items.length; l++) s += createSymbolNode(symbolMap[items[l]], (l - 1) * 16, 16, true);
      } else {
        for (let l = 0; l < 4; l++) s += createSymbolNode(symbolMap[items[l]], l * 16, 0, true);
        for (let l = 4; l < Math.min(items.length, 8); l++) s += createSymbolNode(symbolMap[items[l]], (l - 4) * 16, 16, true);
      }
      document.getElementById("summary" + n).innerHTML = s;
      if (visibleSidebar) {
        for (let l = 0; l < Math.min(items.length, 2); l++) side += createSymbolNode(symbolMap[items[l]], l * 16, 0, true);
        for (let l = 2; l < Math.min(items.length, 4); l++) side += createSymbolNode(symbolMap[items[l]], (l - 2) * 16, 16, true);
  ...
  window.loadDungeon = function (n) {
    ...
    if (dungeonPaths[n].completed) document.getElementById("completedbutton").classList.remove("pointer");
    else document.getElementById("completedbutton").classList.add("pointer");
    updatePathList();
    document.getElementById("dungeonpathsnote").value = dungeonPaths[n].notes;
    if (outstandingUpdate) sendUpdate();
    switchScene("pathlist");
  };

  window.sideLoadDungeon = function (n) {
    currentPath = null;
    if (currentDungeon != -1) saveNotes();
    loadDungeon(n);
  };
  ...
  window.createImportantRoomNodeList = function (type, clickAction) {
    ...
    for (let k = 0; k < dungeonImportant.length; k++)
      if (all ? dungeonImportant[k].priority >= minPriority && dungeonImportant[k].priority <= maxPriority : dungeonImportant[k].dungeon === currentDungeon) starts += createRoomNode(dungeonImportant[k], all, clickAction);
    return starts;
  };
  ...
  window.createUsedRoomNodeList = function (clickAction) {
    ...
    for (let id of items) {
      let room = roomMap[id];
      if (doorshuffle === "C" || room.dungeon === currentDungeon) starts += createRoomNode(room, doorshuffle === "C", clickAction);
    }
    return starts;
  };
  ...
  window.createRoomNode = function (room, showDungeonName = false, clickAction = "startPath", left = 0, top = 0) {
    ...
    if (room.file) {
      ...
      file = "eg1";
      x = (room.supertile % 16) * 128.5 + 0.5;
      y = Math.floor(room.supertile / 16) * 128.5 + 0.5;
      scale = 0.5;
    ...
    return (
      ...
      onClick +
      "'" +
      (left || top ? " style='left: " + left + "px; top: " + top + "px;'" : "") +
      "><img class='roomnodeimg' src='./images/dungeons/" +
      file +
      ".png' style='transform: scale(" +
      scale +
      ") translateX(-" +
      x +
      "px) translateY(-" +
      y +
      "px);'><span class='roomtt'><label class='ttlabel'>" +
      name +
      "</label></span></div>"
    );
  };
  ...
  window.createSymbolNode = function (symbol, left, top, small = false, extraString = null) {
    return (
      ...
      top +
      "px;" +
      (symbol.rotate ? " transform: rotate(" + symbol.rotate + "deg);" : "") +
      "' " +
  ...
  window.moveToTop = function () {
    ...
    if (currentPath != null && currentlyEditing) {
      if (clickedPathIndex === editingIndex) editingIndex = 0;
      else {
        if (clickedPathIndex > editingIndex) editingIndex++;
      }
  ...
  window.moveToBottom = function () {
    ...
    updatePathList();
    if (currentPath != null && currentlyEditing) {
      if (clickedPathIndex === editingIndex) editingIndex = dungeonPaths[currentDungeon].paths.length - 1;
      else {
        if (clickedPathIndex < editingIndex) editingIndex--;
      }
      document.getElementById("dungeonpaths2").childNodes[editingIndex].classList.add("editing");
  ...
  window.deletePath = function () {
    ...
    updatePathList();
    if (currentPath != null && currentlyEditing) {
      if (clickedPathIndex === editingIndex) closePath();
      else {
        if (clickedPathIndex < editingIndex) editingIndex--;
        document.getElementById("dungeonpaths2").childNodes[editingIndex].classList.add("editing");
      }
  ...
  window.startPath = function (path, editing = false) {
    ...
    if (path == "") document.getElementById("deletebutton").classList.add("disabled");
    document.getElementById("forkbutton").classList.remove("disabled");
    if (!globalHasBranch) document.getElementById("forkbutton").classList.add("disabled");
    ...
    for (let k = 0; k < dungeonPaths[currentDungeon].paths.length; k++)
      if (editing && k == editingIndex) pathNodes[k].classList.add("editing");
      else pathNodes[k].classList.remove("editing");
    drawAllSymbols();
  ...
  window.removeLast = function () {
    if (currentPath.length > 1) {
      ...
      document.getElementById("currentpath").innerHTML = drawPath(currentPath, true);
      document.getElementById("deletebutton").classList.remove("disabled");
      if (currentPath == "") document.getElementById("deletebutton").classList.add("disabled");
      document.getElementById("forkbutton").classList.remove("disabled");
      if (!globalHasBranch) document.getElementById("forkbutton").classList.add("disabled");
    }
  };
  ...
  window.closePath = function () {
    updateStartRooms();
    for (let path of document.getElementById("dungeonpaths").childNodes) path.classList.remove("newpath");
    switchScene("pathlist");
    currentPath = null;
  };

  window.savePath = function () {
    if (currentlyEditing) dungeonPaths[currentDungeon].paths[editingIndex] = currentPath;
    else dungeonPaths[currentDungeon].paths.unshift(currentPath);
    updateStartRooms();
    updatePathList();
  ...
  window.updateStatistics = function () {
    ...
    for (let k = 0; k < 13; k++) {
      if (dungeonPaths[k].paths.length != 0) {
        ...
        for (let l = 0; l < dungeonPaths[k].paths.length; l++) nodeCount += Math.floor(dungeonPaths[k].paths[l].length / 2);
      }
      if (dungeonPaths[k].notes.length != 0) noteCount++;
    }
  ...
  window.drawSymbols = function (list, elementID, key) {
    ...
    for (let k = 0; k < list.length; k++) {
      if (doorshuffle !== "C" && omitForeignSymbols && list[k].basic && !list[k].basic.includes(currentDungeon)) continue;
      let id = list[k].id;
      let style = "background-image: url(" + "./images/" + list[k].folder + "/" + list[k].file + ".png);";
      if (list[k].rotate) style += " transform: rotate(" + list[k].rotate + "deg);";
      if (key === "d") style += " position: absolute; left: " + list[k].x + "px; top: " + list[k].y + "px;";
  ...
  window.switchScene = function (scene) {
    ...
    scrollTo(0, 0);
    if (scene === "overview") document.getElementById("homebutton").classList.add("activetab");
    else document.getElementById("homebutton").classList.remove("activetab");
    if (scene === "settings") document.getElementById("sidesettings").classList.add("activetab");
    else document.getElementById("sidesettings").classList.remove("activetab");
  ...
  window.sendUpdate = function () {
    if (!welcomeMode) {
      ...
      try {
        if (enableAutoSave) saveAuto(data, null);
        else document.getElementById("saveauto").classList.remove("disabled");
      } catch (error) {}
      trySaveDisplayData();
  ...
  window.showClickPathModal = function (event, index) {
    ...
    if (!globalHasBranch) document.getElementById("forkbutton2").classList.add("disabled");
    ...
    if (lobbyshuffle) {
      switch (currentDungeon) {
        case 2:
          ...
          break;
        case 5:
          for (let k = 1; k < lobbyEntrances.length; k++) starts += createRoomNode(lobbyEntrances[k], showDungeonName, clickAction);
          for (let k = 0; k < lobbySW.length; k++) starts += createRoomNode(lobbySW[k], showDungeonName, clickAction);
          break;
        case 6:
          starts += createRoomNode(lobbyEntrances[0], showDungeonName, clickAction);
          for (let k = 0; k < lobbyTT.length; k++) starts += createRoomNode(lobbyTT[k], showDungeonName, clickAction);
          break;
        case 11:
          for (let k = 1; k < lobbyEntrances.length; k++) starts += createRoomNode(lobbyEntrances[k], showDungeonName, clickAction);
          for (let k = 0; k < lobbyHC.length; k++) starts += createRoomNode(lobbyHC[k], showDungeonName, clickAction);
          break;
        case 1:
        case 9:
          for (let k = 1; k < lobbyEntrances.length; k++) starts += createRoomNode(lobbyEntrances[k], showDungeonName, clickAction);
      ...
      if ((doorshuffle !== "C" || worldState !== "I") && (currentDungeon === 11 || (doorshuffle === "C" && worldState !== "S"))) starts += createRoomNode(lobbySanctuary, showDungeonName, clickAction);
    } else {
      for (let k = 0; k < dungeonEntrances.length; k++) if (dungeonEntrances[k].dungeon === currentDungeon) starts += createRoomNode(dungeonEntrances[k], showDungeonName, clickAction);
    }
    if (worldState === "S" && currentDungeon === 11) for (let k = 0; k < dungeonStandard.length; k++) starts += createRoomNode(dungeonStandard[k], showDungeonName, clickAction);
    return starts;
  };
  ...
  window.loadRoomModalInit = function () {
    if (importantRoomNodes === "more") loadRoomModalFull();
    if (importantRoomNodes === "default") loadRoomModalDefault();
    if (importantRoomNodes === "fewer") loadRoomModalImportant();
  };
  ...
  window.updateOverworldShuffle = function (showVanillaOWBox) {
    ...
    if (layoutshuffle === "N" && !whirlpoolshuffle && crossedow === "N" && !mixedow) document.getElementById("overworldoptionsfinalbox").style.display = "none";
    else if (showVanillaOWBox) checkAutoAdjustments(false);
  ...
  window.changeMainTrackerSettings = function () {
    if (
      confirm(
        "World State and Entrance Shuffle are handled through the main tracker window.\nEither disable the full sync between the two windows below to change these settings here, or change them in the Mystery menu in the main tracker window.\nProceed if you wish to open the Mystery menu in the main tracker window (even if not in Mystery mode)."
      ) &&
      window.opener &&
  ...
  window.testConnection = function (send) {
    ...
    if (send && window.opener && !window.opener.closed) window.opener.postMessage("PING", "*");
  ...
  window.checkIfResponded = function () {
    ...
    } else document.getElementById("overworldsyncerror").style.display = "none";
  };
  ...
  window.updateGlobalSync = function () {
    ...
    if (document.getElementById("globalsync").checked) {
      ...
      if (window.opener && !window.opener.closed) window.opener.postMessage("ITEMS", "*");
      updateConnectorList();
    } else {
      ...
      awaitingNextUpdate = true;
      if (!document.getElementById("itemsync").checked && !document.getElementById("connectorsync").checked && !document.getElementById("globalsync").checked) document.getElementById("overworldsyncerror").style.display = "none";
      if (window.opener && !window.opener.closed) window.opener.postMessage("RESETLOGIC", "*");
    }
    updateOverviewElements();
    sendUpdate();
  };
  ...
  window.updateConnectorSync = function () {
    ...
    if (!document.getElementById("itemsync").checked && !document.getElementById("connectorsync").checked && !document.getElementById("globalsync").checked) document.getElementById("overworldsyncerror").style.display = "none";
    updateConnectorList();
    if (showFullMapMain && document.getElementById("owmain").style.display === "block") loadFullOverworldMain();
  };
  ...
  window.showGlobalSyncInfo = function () {
    ...
    s +=
      "This only works if the full item tracker in the main window is used. The main window must also be in Overworld Shuffle mode. With Entrance Shuffle, connector caves also have to be set through the map tracker in the main window.\n";
    s +=
      "Also, this window must have been opened from the main tracker window and the main tracker window cannot be reloaded or closed. If a new connection between this window and the main tracker window needs to be established, check out the instructions for information on how to keep your tracking progress.\n";
    s +=
      "Before you start, make sure all mode settings are correctly set in the main tracker window and some form of Overworld Shuffle is enabled (otherwise you may have to go back to the tracker launcher). Only overworld-specific settings are still handled through this window.\n\n";
    s += "Current status: ";
    if (window.opener)
      if (window.opener.closed || awaitingResponse) s += "Disconnected from main tracker window, please save your progress, close this tracker window and then open it again from the main tracker window.";
      else s += "Connected to main tracker window, sync possible.";
    else s += "Not opened from main tracker window, please close this tracker window and open it from the main tracker window obtained through the Community Tracker launcher.";
    alert(s);
  };
  ...
  window.receiveItemUpdate = function (newItems) {
    ...
    if (document.getElementById("globalsync").checked) {
      if (newItems.flags && newItems.flags.hasOwnProperty("gametype")) {
        ...
        if (newState != worldState && (newState === "O" || newState === "S" || newState === "I" || newState === "R")) {
          ...
          if (document.getElementById("owmain").style.display === "block") loadOverworld();
          changed = true;
        }
      }
    ...
    if (document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) {
      let keepConnectors = new Set(),
        createConnectors = [];
      for (let k = 0; k < newItems.connectorOne.length; k++) {
        let id = newItems.connectorOne[k] + "|" + newItems.connectorTwo[k],
          createNew = true;
      ...
      for (let c of createConnectors) createEntranceConnector(getEntranceRegionFromIndex(c[0]), getEntranceRegionFromIndex(c[1]), "unknown", c[2]);
    }
    if (changed) {
  ...
  window.keyDown = function (event) {
    if ((event.target.tagName != "INPUT" || event.target.type != "text") && [32, 33, 34, 35, 36, 38, 40].indexOf(event.keyCode) != -1) event.preventDefault();
  };
  ...
  window.receiveMessage = function (event) {
    if (window.origin === event.origin) {
      ...
      if (event.data.type && event.data.type === "SHUFFLEUPDATE") {
        ...
        // Handle the shuffle update
        if (swapped) {
          clickSwappedScreen(tile);
        } else {
          clickNormalScreen(tile);
        }
      } else if (event.data.dungeonPaths && event.data.dungeonPaths.length === 13) {
        if (loadAllTrack(event.data)) {
          loadOverview();
          if (document.getElementById("itemsync").checked || document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) window.opener.postMessage("ITEMS", "*");
        } else console.log("Error while loading data from main tracker");
      } else if (!isNaN(event.data.tunic)) receiveItemUpdate(event.data);
    }
  };
  ...
  window.allDataTrack = function () {
    ...
    for (let [id, screen] of overworldScreens) {
      for (let [edgeString, edge] of screen.edges) if (edge.out) all.overworldTransitions.push([id, edgeString, edge.out.screen.id, edge.out.string]);
      if (id < 0x40 || id >= 0x80) all.mixedStates.push([id, screen.mixedState]);
    }
    all.items = ownItems;
    ...
    for (let region of customStartRegions) all.customStartRegions.push([region.screen.id, region.name]);
    ...
    for (let checkboxFlag of checkboxFlagsTrack) all[checkboxFlag] = document.getElementById(checkboxFlag).checked;
  ...
  window.loadAllTrack = function (newData) {
    ...
    for (let k = 0; k < 13; k++) if (!(k in newDungeonPaths) || !Array.isArray(newDungeonPaths[k].paths) || !(typeof newDungeonPaths[k].notes === "string" || newDungeonPaths[k].notes instanceof String)) return false;
    dungeonPaths = newDungeonPaths;
    doorshuffle = newData.doorshuffle;
    if (doorshuffle !== "N" && doorshuffle !== "B" && doorshuffle !== "C") doorshuffle = "C";
    ...
    if (!layoutshuffle) layoutshuffle = newData.owshuffle;
    if (layoutshuffle !== "N" && layoutshuffle !== "P" && layoutshuffle !== "F") layoutshuffle = "N";
    clearOverworldTransitions();
    try {
      let overworldTransitions = newData.overworldTransitions;
      if (overworldTransitions) for (let [screenID1, edgeID1, screenID2, edgeID2] of overworldTransitions) connectEdgesByKeys(screenID1, edgeID1, screenID2, edgeID2, false, false);
    } catch (error) {
      console.log(error);
    }
    clearMixedStates();
      ...
      if (mixedStates)
        for (let [id, state] of mixedStates) {
          if (["unknown", "normal", "flipped"].includes("" + state)) {
            ...
            if (screen.parallel) screen.parallel.mixedState = "" + state;
            else if (id === 0x80 || id === 0x82) overworldScreens.get(0x102 - id).mixedState = "" + state;
          }
        }
    } catch (error) {
      console.log(error);
    ...
    customStartRegions = [];
    try {
      for (let regionData of newData.customStartRegions) customStartRegions.push(overworldScreens.get(regionData[0]).regions.get(regionData[1]));
    } catch (error) {
      console.log(error);
    }
    ownItems = newData.items;
    if (!ownItems || typeof ownItems !== "object") ownItems = {};
    ...
    try {
      if (newData.customFluteSpots)
        for (let id of newData.customFluteSpots)
          if (Number.isInteger(id) && id >= 0x00 && id < 0x80 && overworldScreens.has(id)) customFluteSpots.push(id % 0x40);
          else customFluteSpots = [0x30];
    } catch (error) {
      console.log(error);
    }
    ...
    try {
      if (newData.pinnedPaths)
        for (let pathData of newData.pinnedPaths)
          if (Array.isArray(pinnedPaths)) pinnedPaths.push({ path: pathData });
          else if (Array.isArray(pinnedPaths.path)) pinnedPaths.push(pathData);
    } catch (error) {
      console.log(error);
    }
    previousPaths = [];
    try {
      if (newData.previousPaths)
        for (let pathData of newData.previousPaths)
          if (Array.isArray(previousPaths)) previousPaths.push({ path: pathData });
          else if (Array.isArray(previousPaths.path)) previousPaths.push(pathData);
    } catch (error) {
      console.log(error);
    }
    crossedow = newData.crossedow;
    if (crossedow === false) crossedow = "N";
    if (crossedow === true) crossedow = "C";
    if (crossedow !== "N" && crossedow !== "P" && crossedow !== "C") crossedow = "N";
    ...
    if (decoupledow !== "N" && decoupledow !== "O" && decoupledow !== "C") decoupledow = "N";
    ...
    for (let checkboxFlag of checkboxFlagsTrack) document.getElementById(checkboxFlag).checked = newData[checkboxFlag];
    if (showOverworldModeSection !== newData.showOverworldModeSection) toggleOverworldModeSection();
    if (document.getElementById("globalsync").checked) {
      ...
      if (worldState !== "O" && worldState !== "S" && worldState !== "I" && worldState !== "R") worldState = "O";
    ...
    updateMainPathLists();
    document.getElementById("overworldoptionsfinalbox").style.display = "none";
    if (newData.showOptionsFinalBox) checkAutoAdjustments(false);
    if ((document.getElementById("itemsync").checked || document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) && window.opener && !window.opener.closed) window.opener.postMessage("ITEMS", "*");
    return true;
  };
  ...
  window.allDataDisplay = function () {
    ...
    for (let checkboxFlag of checkboxFlagsDisplay) all[checkboxFlag] = document.getElementById(checkboxFlag).checked;
    all.timeString = new Date().toLocaleString();
    all.buildString = window.buildString;
    return all;
  };
  ...
  window.loadAllDisplay = function (newData) {
    if (!newData) return false;
    if (newData.theme && (newData.theme === "dark" || newData.theme === "light")) document.getElementById("settingstheme").value = theme = newData.theme;
    ...
    for (let checkboxFlag of checkboxFlagsDisplay) if (newData[checkboxFlag] === false || newData[checkboxFlag] === true) document.getElementById(checkboxFlag).checked = newData[checkboxFlag];
    ...
    if (!["single", "horizontal", "vertical"].includes(mapModeMain)) mapModeMain = "single";
    mapModePopout = newData.mapModePopout;
    if (!["single", "horizontal", "vertical"].includes(mapModePopout)) mapModePopout = "single";
    ...
    fullZoomMain = newData.fullZoomMain ? Math.max(0.5, Math.min(newData.fullZoomMain, 1.5)) : 0.8;
    fullZoomPopout = newData.fullZoomPopout ? Math.max(0.5, Math.min(newData.fullZoomPopout, 1.5)) : 0.8;
    pathZoom = newData.pathZoom ? Math.max(0.5, Math.min(newData.pathZoom, 2)) : 1;
    fullPathCompact = newData.fullPathCompact;
    try {
      if (restoreWindowSize && newData.outerWidth > 0 && newData.outerHeight > 0) resizeTo(newData.outerWidth, newData.outerHeight);
    } catch (error) {}
  ...
  window.loadManual = function () {
    ...
    if (data && confirm("Load saved data from local storage?" + getLoadText(data)))
      if (loadAllTrack(data)) loadOverview();
      else console.log("Error while loading data from manual save");
  };
  ...
  window.loadAuto = function () {
    ...
    if (data && confirm("Load auto-save data from local storage?" + getLoadText(data)))
      if (loadAllTrack(data)) loadOverview();
      else console.log("Error while loading data from auto-save");
  };

  window.saveAuto = function (data, button) {
    enableAutoSave = true;
    if (!data) data = allDataTrack();
    ...
    input.onchange = (event1) => {
      ...
      reader.onload = (event2) => {
        ...
        if (data && confirm("Load data from the selected file?" + getLoadText(data)))
          if (loadAllTrack(data)) loadOverview();
          else console.log("Error while loading data from JSON file");
      };
  ...
  window.saveFile = function (button) {
    if (tempSaveFile !== null) window.URL.revokeObjectURL(tempSaveFile);
  ...
  window.saveCustomizerYAML = function () {
    if (tempCustomizerFile !== null) window.URL.revokeObjectURL(tempCustomizerFile);
    let s = "meta:\n  players: 1\n";
    s += "settings:\n  1:\n";
    ...
    if (mixedow) {
      ...
      for (let [id, screen] of overworldScreens)
        if (!screen.darkWorld && id !== 0x82) {
          if (screen.mixedState === "normal") normalScreens.push(id);
          if (screen.mixedState === "flipped") swappedScreens.push(id);
        }
      if (normalScreens.length == 0) s += "    force_no_flip: []\n";
      else {
        s += "    force_no_flip:\n";
        for (let id of normalScreens) s += "    - 0x" + id.toString(16).toUpperCase().padStart(2, "0") + " # " + overworldScreens.get(id).randomizerName + "\n";
      }
      if (swappedScreens.length == 0) s += "    force_flip: []\n";
      else {
        s += "    force_flip:\n";
        for (let id of swappedScreens) s += "    - 0x" + id.toString(16).toUpperCase().padStart(2, "0") + " # " + overworldScreens.get(id).randomizerName + "\n";
      }
      s += "    undefined_chance: 50\n";
    }
    if (fluteshuffle) {
      s += "ow-flutespots:\n  1:\n";
      if (customFluteSpots.length == 0) s += "    force: []\n";
      else {
        s += "    force:\n";
        for (let id of customFluteSpots) s += "    - 0x" + id.toString(16).toUpperCase().padStart(2, "0") + " # " + overworldScreens.get(id).randomizerName + "\n";
      }
      s += "    forbid: []\n";
    }
  ...
  window.clearData = function () {
    if (confirm("Delete all currently loaded paths and notes for every dungeon and the overworld?")) {
      dungeonPaths = [];
      for (let k = 0; k < 13; k++) dungeonPaths[k] = { paths: [], notes: "", completed: false };
      clearOverworldTransitions();
      clearMixedStates();
      ownItems = {};
  ...
  window.clearDisplaySettings = function () {
    if (confirm("Reset all display settings?")) {
      ...
      showSidebar();
      if (!showFullMapMain) toggleFullOverworldMain();
      if (!expandedMainTable) toggleOverworldMainTable();
      if (showChecklist) toggleChecklist();
      mapModeMain = mapModePopout = "single";
      mapModeAutoMain = mapModeAutoPopout = true;
      fullZoomMain = fullZoomPopout = 0.8;
  ...
  window.setWelcomeMode = function (enable) {
    welcomeMode = enable;
    if (enable) document.getElementById("app").classList.add("welcomemode");
    else {
      document.getElementById("app").classList.remove("welcomemode");
      if (autoSaveSafety) setTimeout(endAutoSaveSafety, 120000);
      else enableAutoSave = true;
    }
  };
  ...
  window.welcomeStart = function (modeFinal) {
    setWelcomeMode(false);
    scrollTo(0, 0);
    if (document.getElementById("vanillawhirlpoolswelcome").checked) vanillaWhirlpools(null);
    if (document.getElementById("vanillaspecialwelcome").checked) vanillaSpecialScreens(null);
    if (!mixedow)
    ...
    if (!entranceEnabled || worldState === "S") document.getElementById("pinnedlinkshouse").checked = true;
    if (!entranceEnabled && (worldState === "S" || worldState === "I" || doorshuffle === "N")) document.getElementById("pinnedsanctuary").checked = true;
    if (document.getElementById("itemsync").checked || document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) testConnection(true);
    if (modeFinal) vanillaTransitionsMode(null);
    else {
      sendUpdate();
      updateReachableEdges();
    }
  ...
  window.start = function () {
    dungeonPaths = [];
    for (let k = 0; k < 13; k++) dungeonPaths[k] = { paths: [], notes: "", completed: false };
    if (query.door_shuffle) doorshuffle = query.door_shuffle[0];
    if (doorshuffle !== "N" && doorshuffle !== "B" && doorshuffle !== "C") doorshuffle = "C";
    if (query.overworld_shuffle) layoutshuffle = query.overworld_shuffle[0];
    if (layoutshuffle !== "N" && layoutshuffle !== "P" && layoutshuffle !== "F") layoutshuffle = "N";
    if ((query.wild_keys + "").toLowerCase() === "true" || query.world_state === "R" || query.world_state === "r") excludesmallkeys = false;
    if ((query.wild_big_keys + "").toLowerCase() === "true") excludebigkeys = false;
    if (query.world_state) worldState = query.world_state[0];
    if (worldState !== "S" && worldState !== "O" && worldState !== "I" && worldState !== "R") worldState = "O";
    if (query.entrance_shuffle && query.entrance_shuffle !== "N" && query.entrance_shuffle !== "n") entranceEnabled = true;
    let initSync = (query.init_sync + "").toLowerCase() === "true";
    if (initSync || layoutshuffle !== "N") toggleOverworldModeSection();
    ...
    if (!query.request_update) checkAutoAdjustments(false);
    try {
    ...
    if (window.opener && !window.opener.closed) {
      ...
      if (query.request_update) window.opener.postMessage("UPDATE", "*");
      else window.opener.postMessage("ITEMS", "*");
    }
  };
  ...
  window.updateTheme = function () {
    ...
    if (invertDirectionColors) document.body.classList.add("invdir");
    else document.body.classList.remove("invdir");
    for (let k of [2, 5, 8, 11]) directions[k].file = roundDirections ? "arrowrightdownround" : "arrowrightdown";
    for (let k of [1, 4, 7, 10]) directions[k].file = roundDirections ? "arrowrightupround" : "arrowrightup";
    ...
    if (disableBlur) document.body.classList.add("noblur");
    else document.body.classList.remove("noblur");
    updateSyncStatus();
  };
  ...
  window.loadOverworld = function () {
    ...
    if (outstandingUpdate) sendUpdate();
    switchScene("owmain");
    if (showFullMapMain) loadFullOverworldMain();
  };
  ...
  window.loadFullOverworldMain = function () {
    ...
    if (mapModeAutoMain) pickMapModeMain();
    if (fullZoomAutoMain) calculateFullZoomMain();
  };
  ...
  window.updateConnectorList = function () {
    ...
    if (document.getElementById("connectorsync").checked || document.getElementById("globalsync").checked) document.getElementById("owmainaddconnector").classList.add("disabled");
    else document.getElementById("owmainaddconnector").classList.remove("disabled");
    let s = "";
  ...
  window.connectorString = function (connector) {
    ...
    if (regionStart.screen.entranceRegions.length != 1) s += " - " + regionStart.name;
    s += (connector.status === "both" ? " <=> " : connector.status === "unknown" ? " ?=> " : " ==> ") + regionEnd.screen.name;
    if (regionEnd.screen.entranceRegions.length != 1) s += " - " + regionEnd.name;
    return s;
  };
  ...
  window.updateStartRegionList = function () {
    ...
    for (let k = 0; k < customStartRegions.length; k++) {
      ...
      if (region.screen.regions.size != 1) s += " - " + region.name;
  ...
  window.toggleItem = function (item) {
    if (document.getElementById("itemsync").checked || document.getElementById("globalsync").checked) {
      if (window.opener && !window.opener.closed) window.opener.postMessage("TOGGLE " + (item === "gloves" ? "glove" : item), "*");
    } else {
      if (item === "gloves") {
        if (ownItems.mitts) {
          ownItems.gloves = ownItems.mitts = false;
        ...
        if ((ownItems[item] = !ownItems[item])) document.getElementById("item" + item).classList.add("collected");
        else document.getElementById("item" + item).classList.remove("collected");
      }
      outstandingUpdate = true;
      updateReachableEdges();
      drawFullOverworldPanels();
  ...
  window.clickSetFluteSpots = function () {
    if (fluteshuffle) {
      if (mapModePopout === "single") drawDarkWorldPopout = worldState === "I";
  ...
  window.clickScreenFull = function (event, id, main) {
    switch (main ? "editedges" : fullOverworldMode) {
      ...
      case "searchpathstart":
        ...
        return;
      case "editedges":
        if (!preventClickEdgeScreen || !fullOWSelectedEdge || event.detail > 1) updateClickScreenOptions(id);
        return;
      case "editflutespots":
        if (customFluteSpots.includes(id % 0x40)) customFluteSpots.splice(customFluteSpots.indexOf(id % 0x40), 1);
        else {
  ...
  window.updateClickScreenOptions = function (id) {
    ...
    if (fullOWConnectorStart) {
      ...
      if (screen === fullOWSelectedScreen) {
        ...
        if (screen.entranceRegions.length !== 0) document.getElementById(mainString + "newconnector").classList.remove("disabled");
        else document.getElementById(mainString + "newconnector").classList.add("disabled");
  ...
  window.updateClickScreenCommonStarts = function () {
    if (fullOWSelectedScreen) {
      ...
      for (let [regionName, region] of fullOWSelectedScreen.regions)
        if (allStartRegions.includes(region)) s += "<div class='buttonbox small redborder' style='float: left; margin: 2px;' onclick='removeStartRegion(this,\"" + regionName + "\")'>" + regionName + "</div>";
        else s += "<div class='buttonbox small' style='float: left; margin: 2px;' onclick='addStartRegion(this,\"" + regionName + "\")'>" + regionName + "</div>";
  ...
  window.updateEdgeActions = function (main) {
    ...
    if (decoupledow === "N") {
      if (fullOWSelectedEdge.out) {
        ...
        if (fullOWSelectedEdge.in.length) document.getElementById(mainString + "edgecoupleddelete").classList.remove("disabled");
        else document.getElementById(mainString + "edgecoupleddelete").classList.add("disabled");
      }
            ...
            for (let edge of screen.edges.values())
              if (edge.parallel && (!similarow || (!gameLeft && (!gameRight || edge.direction !== "W")))) {
                if ((edge.out && !group.includes(edge.out.screen.id)) || (edge.parallel.out && !group.includes(edge.parallel.out.screen.id))) {
                  let old = edge.out;
                  if (edge.parallel.out) connectEdges(edge, edge.parallel.out, false, false);
                  else deleteConnections(edge, true, false);
                  if (old) connectEdges(edge.parallel, old, false, false);
                  else deleteConnections(edge.parallel, true, false);
                }
                let edgeList1 = [],
                  edgeList2 = [];
                for (let e of edge.in) if (!group.includes(e.screen.id)) edgeList1.push(e);
                for (let e of edgeList1) connectEdges(e, edge.parallel, false, false);
                for (let e of edge.parallel.in) if (!group.includes(e.screen.id) && !edgeList1.includes(e)) edgeList2.push(e);
                for (let e of edgeList2) connectEdges(e, edge, false, false);
              }
          }
      }
  ...
  window.removeStartRegion = function (button, regionName) {
    ...
    if (fixedStartRegions.includes(region)) {
      if (fullOWSelectedScreen.id % 0x40 === 0x2c) document.getElementById("pinnedlinkshouse").checked = false;
      if (fullOWSelectedScreen.id % 0x40 === 0x13) document.getElementById("pinnedsanctuary").checked = false;
      if (fullOWSelectedScreen.id % 0x40 === 0x03) document.getElementById("pinnedoldman").checked = false;
      if (fullOWSelectedScreen.id % 0x40 === 0x1b) document.getElementById("pinnedpyramid").checked = false;
    }
    for (let k = 0; k < customStartRegions.length; k++)
  ...
  window.fullOverworldRepeatSearch = function () {
    if (fullOWPath && fullOWPath.search) loadOverworldSearch(fullOWPath.search);
  };
  ...
  window.drawDirectionsFromScreen = function (screen) {
    ...
    if (screen.portal) s += drawDirectionFromSymbol(overworldEdgeToDirection["PO"], 'appendToOverworldPath("PO")');
    if (screen.darkWorld) s += drawDirectionFromSymbol(overworldEdgeToDirection["MI"], 'appendToOverworldPath("MI")');
  ...
  window.drawDirectionFromSymbol = function (symbol, onClick) {
    ...
    if (symbol.rotate) style += " transform: rotate(" + symbol.rotate + "deg);";
  ...
  window.appendToOverworldPath = function (edgeString) {
    ...
    if (edgeString === "PO" || edgeString === "MI") {
      ...
      let edge = currentOWScreen.edges.get(edgeString),
        targetEdge = null;
      if ((edgeString === "ZW" ? !whirlpoolshuffle : layoutshuffle === "N") && (crossedow !== "C" || !edge.parallel)) targetEdge = getConnectedEdge(edge, false);
      if (!targetEdge && edge.out && document.getElementById("alwaysfollowmarked").checked) targetEdge = edge.out;
      if (targetEdge) {
        ...
        if (mapModePopout === "single" && crossedow === "N") drawDarkWorldPopout = isDarkWorld(currentOWScreen);
  ...
  window.selectTargetScreen = function (id) {
    ...
    if (validEdges.length > 1) {
      ...
      for (let edge of validEdges) options[edge.symbol] = true;
  ...
  window.connectorBackToFullOverworld = function () {
    ...
    if (!useMain) document.getElementById("fullowModal").style.display = "block";
    drawFullOverworldPanels();
  };
        ...
        case 0: //Overworld screen
          if (detailed || k == 0 || k == path.length - 1) {
            ...
            let name = screen.name,
              scale = 0.5,
              classes = "owscreen single";
            if (screen.big) scale = 0.25;
            ...
            s +=
              ...
              "px; top: " +
              top +
              "px;' onmouseover='hoverPathNumber(" +
              path[k] +
              ")' onmouseout='clearPathNumber()'><img class='roomnodeimg' src='./images/overlay/" +
              file +
              ".png' style='transform: translateX(-" +
              x +
              "px) translateY(-" +
              y +
              "px) scale(" +
              scale +
              ");'><span class='roomtt'><label class='ttlabel'>" +
              name +
              "</label></span></div>";
            if (drawNumbers) s += "<div class='pathnumber' style='left: " + (left + 24) + "px; top: " + (top + 24) + "px;'>" + k / 3 + "</div>";
            if (k != 0 && !specialEdgeTypes.includes(path[k - 1])) {
              ...
              if (detailed || path.length <= 4) {
                let minX = Math.min(lastEdgeX, newX),
                  maxX = Math.max(lastEdgeX, newX);
                let minY = Math.min(lastEdgeY, newY),
                  maxY = Math.max(lastEdgeY, newY);
  ...
  window.saveCurrentOverworldPath = function () {
    for (let k = 3; k < currentOWPath.length; k += 3)
      if (!specialEdgeTypes.includes(currentOWPath[k - 2])) {
        ...
        let edge1 = screen1.edges.get(currentOWPath[k - 2]),
          edge2 = screen2.edges.get(currentOWPath[k - 1]);
  ...
  window.saveOverworldSearch = function () {
    ...
    if (searchStartScreen) {
      ...
      if (searchStartRegion) search.startRegion = searchStartRegion.name;
    }
    if (searchAddCommonStarts) search.startMode = "canstartcommon";
    else if (!searchSaveQuitFluteEdges) search.startMode = "noflutequit";
    else search.startMode = "onlystartscreen";
    search.targetScreen = currentOWScreen.id;
    if (searchTargetRegion) search.targetRegion = searchTargetRegion.name;
  ...
  window.setSearchStartMode = function (startMode) {
    if (startMode === "canstartcommon") searchCanStartCommon();
    else if (startMode === "noflutequit") searchNoFluteQuit();
    else searchOnlyStartScreen();
  };
  ...
  window.searchSwapStartTarget = function () {
    if (searchStartScreen) {
      let oldScreen = searchStartScreen,
        oldRegion = searchStartRegion;
      searchStartScreen = currentOWScreen;
      searchStartRegion = searchTargetRegion;
  ...
  window.searchOverworldPath = function (resetStartRegions, resetTargetRegions) {
    ...
    if (document.getElementById("owsearchignoreitemrules").checked) giveAllItems(items);
    ...
    if (searchStartScreen && searchAddCommonStarts) startRegions = startRegions.concat(allStartRegions, fluteSpotRegions);
    let options = {};
    if (document.getElementById("owsearchfollower").value !== "none") options.follower = document.getElementById("owsearchfollower").value;
    ...
    if (searchStartScreen) document.getElementById("owsearchswapbutton").classList.remove("disabled");
    else document.getElementById("owsearchswapbutton").classList.add("disabled");
    ...
    if (searchStartScreen && searchStartScreen.regions.size != 1) {
      ...
      for (let name of searchStartScreen.regions.keys()) s += "<div id='owstartregion" + name + "' class='cell buttonbox dense' onclick='setSearchStartRegion(\"" + name + "\")'>" + name + "</div>";
    ...
    if (screen.regions.size != 1) {
      ...
      for (let name of screen.regions.keys()) s += "<div id='owregion" + name + "' class='cell buttonbox dense' onclick='setSearchTargetRegion(\"" + name + "\")'>" + name + "</div>";
  ...
  window.setSearchStartRegion = function (regionName) {
    ...
    if (regionName) {
      ...
      for (let r of searchStartScreen.regions.values())
        if (r === searchStartRegion) document.getElementById("owstartregion" + r.name).classList.add("selected");
        else document.getElementById("owstartregion" + r.name).classList.remove("selected");
    } else {
      document.getElementById("owstartanyregion").classList.add("selected");
      for (let r of searchStartScreen.regions.values()) document.getElementById("owstartregion" + r.name).classList.remove("selected");
    }
    searchOverworldPath(false, false);
  };
  ...
  window.updateSearchResults = function () {
    let paths = [],
      region = searchTargetRegion;
    if (region) {
      for (let path of searchResults.get(region)) paths.push(path);
      if (currentOWScreen.regions.size != 1) {
        ...
        for (let r of searchResults.keys())
          if (r === region) document.getElementById("owregion" + r.name).classList.add("selected");
          else document.getElementById("owregion" + r.name).classList.remove("selected");
      }
    } else {
      for (let list of searchResults.values()) for (let path of list) paths.push(path);
      if (currentOWScreen.regions.size != 1) {
        document.getElementById("owanyregion").classList.add("selected");
        for (let r of searchResults.keys()) document.getElementById("owregion" + r.name).classList.remove("selected");
      }
    }
  ...
  window.pinOverworldPath = function (button) {
    ...
    outstandingUpdate = true;
    if (clickedOverworldPathListName !== "search") updateMainPathLists();
    buttonFlash(button);
  };

  window.repeatSearch = function () {
    if (clickedOverworldPath && clickedOverworldPath.search) loadOverworldSearch(clickedOverworldPath.search);
  };
  ...
  window.equalPaths = function (path1, path2) {
    ...
    for (let k = 0; k < path1.length; k++) if (path1[k] != path2[k]) return false;
    return true;
  };
  ...
  window.updateItemTracker = function () {
    for (let item of ["moonpearl", "hammer", "hookshot", "flippers", "mirror", "flute", "boots", "book", "lantern", "agahnim"])
      if (ownItems[item]) document.getElementById("item" + item).classList.add("collected");
      else document.getElementById("item" + item).classList.remove("collected");
    if (ownItems.gloves)
      if (ownItems.mitts) document.getElementById("itemgloves").classList.add("collected", "mitts");
      else {
  ...
  window.drawFullOverworldPanels = function () {
    if (showFullMapMain && document.getElementById("owmain").style.display === "block") drawFullOverworldPanel(true);
    if (!useMain) drawFullOverworldPanel(false);
  };
  ...
  window.drawFullOverworldPanel = function (main) {
    ...
    for (let [id, screen] of overworldScreens) {
      ...
      if ((mapMode === "single" && darkWorld != drawDarkWorld && (!mixedow || !screen.special || screen.mixedState !== "unknown")) || (screen.special && mode === "editflutespots")) continue;
      let unknownScreen = mixedow && screen.mixedState === "unknown" && mode !== "editflutespots";
      let scale = 0.5,
        classes = "owscreen full",
        onClick = unknownScreen ? "" : "clickScreenFull(event," + id + "," + main + ")",
        valid = true;
      if (screen.big) classes += " bigscreen";
      if (screen === fixedScreen) classes += " active";
      if (fullOWConnectorStart) valid = screen.entranceRegions.length;
      if (valid && fixedEdge) {
        valid = false;
        if (!unknownScreen)
          for (let edge of screen.edges.values())
      ...
      if (mode === "editedges" && !fullOWSelectedScreen && !fullOWSelectedEdge && !fullOWConnectorStart && fullOWPath) {
        ...
        if (indexInPath === -1) valid = false;
      }
      if (!valid) classes += " gray";
      if (unknownScreen) classes += " unknown";
      let x = screen.special ? screen.x : (id % 8) * 128 * scale;
      let y = screen.special ? screen.y : (id % 0x40 >> 3) * 128 * scale;
      if (mapMode === "horizontal" && darkWorld != drawDarkWorld) x += 1024 * scale;
      if (mapMode === "vertical" && darkWorld != drawDarkWorld) y += 1024 * scale;
      s +=
        "<div id='full" +
        (main ? "main" : "popout") +
        id +
        ...
        "' onclick='" +
        onClick +
        (unknownScreen || (layoutshuffle === "N" && !whirlpoolshuffle && crossedow !== "C") ? "' " : "' onmouseover='hoverScreen(" + id + "," + main + ")' onmouseout='clearFullOverlay(" + main + ")' ");
      if (screen.special) s += "style='left: " + x + "px; top: " + y + "px; z-index: 1'><div class='specialshadow'><img class='roomnodeimg' src='./images/overlay/" + screen.file + ".png' style='transform: scale(" + scale + ");'></div>";
      else
        s +=
          "style='left: " +
          x +
          "px; top: " +
          y +
          "px;'><img class='roomnodeimg' src='./images/overlay/" +
          (screen.darkWorld ? "dark" : "light") +
          "world.png' style='transform: translateX(-" +
          (x % (1024 * scale)) +
          "px) translateY(-" +
          (y % (1024 * scale)) +
          "px) scale(" +
      ...
      s += "<span class='roomtt'>";
      if (unknownScreen) {
        let left = getBigScreenSubareaX(screen),
          top = getBigScreenSubareaY(screen);
        ...
        s +=
          "</span><span class='questionmark" +
          (checkableScreensGlobal.has(id & 0xbf)
            ? " greentext"
            : checkableScreensDarkRoomsGlobal.has(id & 0xbf)
            ? " bluetext"
            : maybeCheckableScreensGlobal.has(id & 0xbf)
            ? " yellowtext"
            : maybeCheckableScreensDarkRoomsGlobal.has(id & 0xbf)
            ? " purpletext"
            : "") +
          "'" +
          (top ? " style='left: " + left + "px; top: " + top + "px;' " : "") +
          ">?";
      } else s += "<label class='ttlabel'" + (x + y ? "" : " style='line-height: 144px;'") + ">" + screen.name + "</label>";
      s += "</span></div>";
      if (drawSaveQuitSpots === "on" || (drawSaveQuitSpots === "startshuffle" && (entranceEnabled || (doorshuffle !== "N" && worldState !== "S" && worldState !== "I"))))
        for (let region of screen.regions.values())
          if (allStartRegions.includes(region)) {
            s += "<div class='savequitsymbol' style='left: " + (x + getBigScreenSubareaX(screen) + 16) + "px; top: " + (y + getBigScreenSubareaY(screen) + 16) + "px; opacity: " + (mode === "editflutespots" ? 0.25 : 0.5) + "'></div>";
            break;
          }
      if (
        (mode === "editflutespots" || ((drawFluteSpots === "on" || (drawFluteSpots === "fluteshuffle" && fluteshuffle)) && ownItems.flute >= 1 && document.getElementById("activeflutebox").checked)) &&
        darkWorld === (worldState === "I") &&
        (fluteshuffle ? customFluteSpots : vanillaFluteSpots).includes(id % 0x40)
      )
        s += "<div class='flutesymbol' style='left: " + (x + getBigScreenSubareaX(screen) + 16) + "px; top: " + (y + getBigScreenSubareaY(screen) + 16) + "px; opacity: " + (mode === "editflutespots" ? 1 : 0.75) + "'></div>";
      if (!fullOWConnectorStart && !unknownScreen && mode !== "editflutespots")
        for (let edge of screen.edges.values()) {
          ...
          if ((isShuffled || drawUnshuffledEdges) && (!fixedEdge || edge === fixedEdge || edgesCompatible(edge, fixedEdge))) {
            ...
            if (edge === fixedEdge || edge === fullOWSelectedEdge) classes += " active";
            if (isShuffled) {
              if (edge.out) {
                if (darkWorld !== isDarkWorld(edge.out.screen)) classes += " owedgecrossed";
                if (mixedow && edge.out.screen.mixedState === "unknown") questionMark = true;
              }
            } else {
              classes += " unshuffled";
              ...
              if (!e) {
                if (crossedow !== "N") {
                  if (crossedow === "C" && swappedow && edge.parallel) classes += " owedgecrossed";
                  else {
                    questionMark = true;
                    ...
                    if (f && darkWorld !== isDarkWorld(f.screen)) classes += " owedgecrossed";
                  }
                }
              } else {
                if (darkWorld !== isDarkWorld(e.screen)) classes += " owedgecrossed";
                if (mixedow && edge.vanilla.screen.mixedState === "unknown" && !edge.parallel) questionMark = true;
              }
            }
            if (visitedScreenEdges.has(edge)) classes += edge.out || !isShuffled ? " turqoise" : " green";
            else classes += edge.out || !isShuffled ? " blue" : " red";
            s += "<div class='" + classes + "' style='left: " + (x + edge.x * 128 * scale - 8) + "px; top: " + (y + edge.y * 128 * scale - 8) + "px;" + (screen.special ? " z-index: 1;" : "") + "'";
            if (mode === "editedges" && isShuffled) s += " onclick='clickEdgeFull(" + id + ',"' + edge.string + '",' + main + ")' onmouseover='hoverEdge(" + id + ',"' + edge.string + "\")' onmouseout='clearFullOverlay(" + main + ")'";
            ...
            if (edge.in.length && (decoupledow !== "N" || edge.in.length > 1 || edge.out !== edge.in[0])) {
              ...
              s +=
                ...
                classes +
                "' style='left: " +
                (x + edge.x * 128 * scale - 4) +
                "px; top: " +
                (y + edge.y * 128 * scale - 4) +
                "px;" +
                (screen.special ? " z-index: 1;" : "") +
                "'>" +
                (decoupledow === "C" || edge.in.length > 1 ? edge.in.length : "") +
                "</div>";
            }
          }
        }
      if ((id === 0x18 || (id === 0x58 && mapMode === "single")) && mode === "editedges" && !fullOWConnectorStart && ownItems.flute >= 1 && !document.getElementById("activeflutebox").checked)
        s +=
          "<span class='activateflute" +
          (visitedScreenEdges.has(overworldScreens.get(0x18).edges.get("N0")) && (ownItems.moonpearl || (id === 0x18) === (darkWorld === (worldState === "I"))) ? " green" : "") +
          "' style='left: " +
          (x + 16) +
          "px; top: " +
          (y + 26) +
          "px;' onclick='clickFluteActivated()'>Flute activated</span>";
      if (
        id % 0x40 === 0x18 &&
        (darkWorld === (worldState === "I") || mapMode === "single") &&
        mode === "editedges" &&
        !fullOWConnectorStart &&
        fluteshuffle &&
        customFluteSpots.length != 8 &&
        ownItems.flute >= 1 &&
        document.getElementById("activeflutebox").checked
      )
      ...
      if (id % 0x40 === 0x30 && mixedow && !darkWorld) s += "<div class='lightworldsymbol' style='left: " + (x + 1) + "px; top: " + (y + 47) + "px;'></div>";
      if (id % 0x40 === 0x30 && mixedow && darkWorld) s += "<div class='darkworldsymbol' style='left: " + (x + 1) + "px; top: " + (y + 47) + "px;'></div>";
      if (indexInPath !== -1)
      ...
      for (let k = 0; k < fullOWPath.path.length; k += 3) {
        if (k != 0 && !specialEdgeTypes.includes(fullOWPath.path[k - 1])) {
          let screen1 = overworldScreens.get(fullOWPath.path[k - 3]),
            screen2 = overworldScreens.get(fullOWPath.path[k]),
            scale = 0.5;
          ...
          if (mapMode === "horizontal" && isDarkWorld(screen1) != drawDarkWorld) x += 1024 * scale;
          if (mapMode === "vertical" && isDarkWorld(screen1) != drawDarkWorld) y += 1024 * scale;
          s += drawFullEdgeConnection(edge1, edge2, x, y, false, mapMode != "single" || isDarkWorld(screen1) === isDarkWorld(screen2) ? 1 : 0.5);
          ...
          if (mapMode === "horizontal" && isDarkWorld(screen2) != drawDarkWorld) x += 1024 * scale;
          if (mapMode === "vertical" && isDarkWorld(screen2) != drawDarkWorld) y += 1024 * scale;
    ...
    if (!main && mode === "editflutespots") document.getElementById(mainString + "flutespotstext").innerHTML = customFluteSpots.length + "/8 flute spots currently set";
  };
  ...
  window.drawSingleOverworldScreen = function (screen, mode) {
    ...
    let s = "";
    let scale = 1,
      classes = mode === "search" ? "owscreen single" : "bigowscreen single";
    if (screen.big) scale = 0.5;
    if (mode === "search") scale /= 2;
    ...
    s +=
      ...
      "px; top: " +
      0 +
      "px;'><img class='bigroomnodeimg' src='./images/overlay/" +
      file +
      ".png' style='transform: translateX(-" +
      x +
      "px) translateY(-" +
    ...
    if (mode !== "connector" && mode !== "search")
      for (let [edgeString, edge] of screen.edges)
        if (mode === "alledges" || (edge.direction === extraOWDirection && (terrainow || !edge.water))) {
          ...
          if (visitedScreenEdges.has(edge)) classes += edge.out ? " turqoise" : " green";
          else classes += edge.out ? " blue" : " red";
  ...
  window.drawFullEdgeConnection = function (edge1, edge2, x1, y1, gray, opacity, main, invert = false) {
    ...
    if ((main ? mapModeMain === "horizontal" : mapModePopout === "horizontal") && isDarkWorld(edge2.screen) != (main ? drawDarkWorldMain : drawDarkWorldPopout)) newX += 1024 * scale;
    if ((main ? mapModeMain === "vertical" : mapModePopout === "vertical") && isDarkWorld(edge2.screen) != (main ? drawDarkWorldMain : drawDarkWorldPopout)) newY += 1024 * scale;
    let minX = Math.min(lastEdgeX, newX),
      maxX = Math.max(lastEdgeX, newX);
    let minY = Math.min(lastEdgeY, newY),
      maxY = Math.max(lastEdgeY, newY);
    return (
      "<div class='crossed" +
      (gray ? "gray" : "") +
      (lastEdgeY > newY != lastEdgeX > newX ? "left" : "right") +
      "' style='left: " +
      minX +
      "px; top: " +
      minY +
  ...
  window.hoverScreen = function (id, main) {
    if (main || fullOverworldMode !== "editflutespots") {
      ...
      let y = screen.special ? screen.y : (id % 0x40 >> 3) * 128 * scale;
      if ((main ? mapModeMain === "horizontal" : mapModePopout === "horizontal") && isDarkWorld(screen) != (main ? drawDarkWorldMain : drawDarkWorldPopout)) x += 1024 * scale;
      if ((main ? mapModeMain === "vertical" : mapModePopout === "vertical") && isDarkWorld(screen) != (main ? drawDarkWorldMain : drawDarkWorldPopout)) y += 1024 * scale;
      for (let edge of screen.edges.values())
        if (isShuffledEdge(edge)) {
          if (edge.out) {
            s += drawFullEdgeConnection(edge, edge.out, x, y, false, (main ? mapModeMain !== "single" : mapModePopout !== "single") || isDarkWorld(screen) === isDarkWorld(edge.out.screen) ? 1 : 0.5, main);
            let element = document.getElementById("full" + (main ? "main" : "popout") + edge.out.screen.id);
            if (element) element.classList.add("connectedout");
          }
          for (let e of edge.in) {
            if (decoupledow !== "N" || edge.in.length > 1 || e !== edge.out)
              s += drawFullEdgeConnection(edge, e, x, y, true, (main ? mapModeMain !== "single" : mapModePopout !== "single") || isDarkWorld(screen) === isDarkWorld(e.screen) ? 1 : 0.5, main, true);
            let element = document.getElementById("full" + (main ? "main" : "popout") + e.screen.id);
            if (element) element.classList.add("connectedin");
          }
        }
  ...
  window.hoverEdge = function (id, edgeString) {
    let screen = overworldScreens.get(id),
      edge = screen.edges.get(edgeString);
  ...
  window.hoverEdgeInSingle = function (id, edgeString, index) {
    let screen = overworldScreens.get(id),
      edge = screen.edges.get(edgeString);
    if (index < edge.in.length) drawEdgeFullOverlay(screen, edge, false, true, edge.in[index], useMain);
  };
  ...
  window.clearFullOverlay = function (main) {
    ...
    for (let element of document.getElementById(main ? "fullowmainpanelmain" : "fullowpanelmain").childNodes) element.classList.remove("connectedout", "connectedin");
  };
  ...
  window.showFullOverworldModal = function (mode) {
    ...
    if (mode === "newpath" || mode === "searchpath") document.getElementById("fullowpanel").classList.add("pointer");
    else document.getElementById("fullowpanel").classList.remove("pointer");
    if (mode === "editedges") document.getElementById("fullowpanel").classList.add("clickedges");
    else document.getElementById("fullowpanel").classList.remove("clickedges");
    ...
    document.getElementById("fullowModal").style.display = "block";
    if (mapModeAutoPopout) pickMapModePopout();
    if (fullZoomAutoPopout) calculateFullZoomPopout();
  };
  ...
  window.hideFullOverworldModal = function () {
    ...
    document.getElementById("fullowModal").style.display = "none";
    if (fullOWFixedEdge) {
      while (currentOWPath.length % 3 > 1) currentOWPath.pop();
      updateCurrentOverworldPath();
    }
    if (outstandingUpdate) sendUpdate();
  };
  ...
  window.showOverworldConnectorModal = function () {
    ...
    if (fullOWConnectorStart.entranceRegions.length === 1) document.getElementById("cstart" + fullOWConnectorStart.entranceRegions[0].name.replace(/\s+/g, "")).checked = true;
    s = "";
    ...
    if (fullOWConnectorEnd.entranceRegions.length === 1) document.getElementById("cend" + fullOWConnectorEnd.entranceRegions[0].name.replace(/\s+/g, "")).checked = true;
  ...
  window.hideOverworldConnectorModal = function () {
    if (!backToMain) fullOWConnectorStart = fullOWConnectorEnd = null;
    if (showFullMapMain && document.getElementById("owmain").style.display === "block") loadFullOverworldMain();
    document.getElementById("owconnectorModal").style.display = "none";
  };
  ...
  window.showOverworldExtraModal = function (options) {
    ...
    for (let k = 0; k < 3; k++)
      if (options[k]) document.getElementById("edgebutton" + k).classList.remove("disabled");
      else document.getElementById("edgebutton" + k).classList.add("disabled");
  ...
  window.showClickOverworldPathModal = function (event, pathIndex, listName) {
    ...
    for (let k = 0; k < clickedOverworldPath.length; k += 3) clickedOverworldPath[k] = parseInt(clickedOverworldPath[k]);
  ...
  window.showClickConnectorModal = function (event, index) {
    ...
    if (connector.status === "single") document.getElementById("clickedconnectorone").classList.add("disabled");
    else document.getElementById("clickedconnectorone").classList.remove("disabled");
    if (connector.status === "both") document.getElementById("clickedconnectortwo").classList.add("disabled");
    else document.getElementById("clickedconnectortwo").classList.remove("disabled");
    if (connector.status === "unknown") document.getElementById("clickedconnectorunknown").classList.add("disabled");
    else document.getElementById("clickedconnectorunknown").classList.remove("disabled");
  ...
  window.updateZoomEditMapModeModal = function () {
    ...
    if (fullZoom <= 0.5) document.getElementById("mapmodezoomout").classList.add("disabled");
    else document.getElementById("mapmodezoomout").classList.remove("disabled");
    if (fullZoom >= 1.5) document.getElementById("mapmodezoomin").classList.add("disabled");
    else document.getElementById("mapmodezoomin").classList.remove("disabled");
  };
  ...
  window.updateConnectorButton = function () {
    if (document.querySelector("input[name='connectorstartgroup']:checked") && document.querySelector("input[name='connectorendgroup']:checked")) document.getElementById("saveconnector").classList.remove("disabled");
    else document.getElementById("saveconnector").classList.add("disabled");
  };
  ...
  window.pickMapModeMain = function () {
    ...
    if (Math.max(0.5, Math.min(maxWidth / 1024, (maxHeight + (hideTopBar ? 34 : 0)) / 512)) >= Math.min((singleZoom * 7) / 8, 0.75)) mapModeMain = "horizontal";
    else if (Math.max(0.5, Math.min(maxWidth / 512, (maxHeight + (hideTopBar ? 34 : 0)) / 1024)) >= Math.min((singleZoom * 7) / 8, 0.75)) mapModeMain = "vertical";
    else mapModeMain = "single";
    if (mapModeMain != oldMapMode) {
      if (document.getElementById("editmapmodeModal").style.display === "block" && editMapModeMain) document.getElementById("mapmode" + mapModeMain).checked = true;
      if (fullZoomAutoMain) calculateFullZoomMain();
      drawFullOverworldPanels();
      updateZoomEditMapModeModal();
      updateTopBars();
  ...
  window.pickMapModePopout = function () {
    let maxWidth = document.body.clientWidth - 14,
      maxHeight = window.innerHeight - 212;
    let singleZoom = Math.max(256, Math.min(Math.min(maxWidth, maxHeight), 768)) / 512;
    let oldMapMode = mapModePopout;
    if (Math.max(0.5, Math.min(maxWidth / 1024, (maxHeight + (hideTopBar ? 34 : 0)) / 512)) >= Math.min((singleZoom * 7) / 8, 0.75)) mapModePopout = "horizontal";
    else if (Math.max(0.5, Math.min(maxWidth / 512, (maxHeight + (hideTopBar ? 34 : 0)) / 1024)) >= Math.min((singleZoom * 7) / 8, 0.75)) mapModePopout = "vertical";
    else mapModePopout = "single";
    if (mapModePopout != oldMapMode) {
      if (document.getElementById("editmapmodeModal").style.display === "block" && !editMapModeMain) document.getElementById("mapmode" + mapModePopout).checked = true;
      if (fullZoomAutoPopout) calculateFullZoomPopout();
      drawFullOverworldPanels();
      updateZoomEditMapModeModal();
      updateTopBars();
  ...
  window.zoomOutFullOverworldMain = function () {
    ...
    outstandingUpdate = true;
    if (fullZoomMain > 0.5) {
      fullZoomMain = Math.ceil(fullZoomMain * 20 - 0.0001) / 20 - 0.05;
      applyFullOverworldZoomMain();
      document.getElementById("mainzoominow").classList.remove("disabled");
      if (fullZoomMain <= 0.5) document.getElementById("mainzoomoutow").classList.add("disabled");
      else document.getElementById("mainzoomoutow").classList.remove("disabled");
    }
  };
  ...
  window.zoomOutFullOverworldPopout = function () {
    ...
    outstandingUpdate = true;
    if (fullZoomPopout > 0.5) {
      fullZoomPopout = Math.ceil(fullZoomPopout * 20 - 0.0001) / 20 - 0.05;
      applyFullOverworldZoomPopout();
      document.getElementById("zoominow").classList.remove("disabled");
      if (fullZoomPopout <= 0.5) document.getElementById("zoomoutow").classList.add("disabled");
      else document.getElementById("zoomoutow").classList.remove("disabled");
    }
  };
  ...
  window.zoomInFullOverworldMain = function () {
    ...
    outstandingUpdate = true;
    if (fullZoomMain < 1.5) {
      fullZoomMain = Math.floor(fullZoomMain * 20 + 0.0001) / 20 + 0.05;
      applyFullOverworldZoomMain();
      document.getElementById("mainzoomoutow").classList.remove("disabled");
      if (fullZoomMain >= 1.5) document.getElementById("mainzoominow").classList.add("disabled");
      else document.getElementById("mainzoominow").classList.remove("disabled");
    }
  };
  ...
  window.zoomInFullOverworldPopout = function () {
    ...
    if (fullZoomPopout < 1.5) {
      fullZoomPopout = Math.floor(fullZoomPopout * 20 + 0.0001) / 20 + 0.05;
      applyFullOverworldZoomPopout();
      document.getElementById("zoomoutow").classList.remove("disabled");
      if (fullZoomPopout >= 1.5) document.getElementById("zoominow").classList.add("disabled");
      else document.getElementById("zoominow").classList.remove("disabled");
    }
  };

  window.resizeHandler = function () {
    updateOverworldMainColumns();
    if (mapModeAutoMain && showFullMapMain && document.getElementById("owmain").style.display === "block") pickMapModeMain();
    if (fullZoomAutoMain && showFullMapMain && document.getElementById("owmain").style.display === "block") calculateFullZoomMain();
    if (mapModeAutoPopout && !useMain) pickMapModePopout();
    if (fullZoomAutoPopout && !useMain) calculateFullZoomPopout();
    if (document.getElementById("editmapmodeModal").style.display == "block") updateZoomEditMapModeModal();
  };

  window.updateOverworldMainColumns = function () {
    if (overworldMainColumns === "2" || (overworldMainColumns === "auto" && document.body.clientWidth - (visibleSidebar ? 72 : 0) >= 850)) document.getElementById("owmaincolumncontainer").classList.add("columns2");
    else document.getElementById("owmaincolumncontainer").classList.remove("columns2");
  };

  window.calculateFullZoomMain = function () {
    let maxWidth = document.body.clientWidth - (visibleSidebar ? 72 : 0),
      maxHeight = window.innerHeight - 252 + (hideTopBar && mapModeMain !== "single" ? 34 : 0);
    if (mapModeMain === "horizontal") maxWidth /= 2;
    if (mapModeMain === "vertical") maxHeight /= 2;
  ...
  window.calculateFullZoomPopout = function () {
    ...
    if (mapModePopout === "horizontal") maxWidth /= 2;
    if (mapModePopout === "vertical") maxHeight /= 2;
    let size = Math.max(256, Math.min(Math.min(maxWidth, maxHeight), 768));
  ...
  window.zoomOutOverworldPath = function () {
    pathZoom = Math.max(0.5, Math.ceil(pathZoom * 8 - 0.0001) / 8 - 0.125);
    outstandingUpdate = true;
    applyOverworldPathZoom();
  };

  window.zoomInOverworldPath = function () {
    pathZoom = Math.min(Math.floor(pathZoom * 8 + 0.0001) / 8 + 0.125, 2);
    outstandingUpdate = true;
    applyOverworldPathZoom();
  };
  ...
  window.getModifiedOwnItems = function () {
    ...
    if (!document.getElementById("activeflutebox").checked) items.flute = 0;
  ...
  window.updateReachableEdges = function () {
    ...
    if (document.getElementById("pinnedlinkshouse").checked) inspectStartRegion(0x2c, "Main", false);
    if (document.getElementById("pinnedsanctuary").checked) inspectStartRegion(worldState === "I" ? 0x53 : 0x13, "Main", true);
    if (document.getElementById("pinnedoldman").checked) inspectStartRegion(0x03, "Bottom", true);
    if (document.getElementById("pinnedpyramid").checked) inspectStartRegion(0x5b, "Balcony", false);
    if (items.flute >= 1) for (let id of fluteshuffle ? customFluteSpots : vanillaFluteSpots) inspectStartRegion(id, null, false);
    ...
    if (layoutshuffle !== "N" || whirlpoolshuffle || crossedow === "C") {
      let c = 0;
      for (let edge of r.visitedScreenEdges) if (!edge.out && isShuffledEdge(edge)) c++;
  ...
  window.inspectStartRegion = function (screenID, regionName, ignoreSwaps) {
    ...
    if (mixedow) {
      if (screen.mixedState === "unknown") {
        ...
        let keyNormal = id + " normal",
          keySwapped = id + " swapped";
        if (!continueRegionsGlobal.has(keyNormal)) continueRegionsGlobal.set(keyNormal, new Set());
        if (!continueRegionsGlobal.has(keySwapped)) continueRegionsGlobal.set(keySwapped, new Set());
